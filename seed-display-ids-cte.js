...
async function seedDisplayIdsWithCTE() {
  try {
    console.log('Seeding display IDs using CTE approach...');
    
    // Get users without displayId and assign them
    const [usersWithoutId] = await sequelize.query(`
      SELECT id, email FROM users WHERE displayid IS NULL ORDER BY created_at
    `);
    ...
    const [employeesWithoutId] = await sequelize.query(`
      SELECT id, "firstName", "lastName" FROM employees WHERE displayid IS NULL ORDER BY created_at
    `);
    ...
    const [studentsWithoutId] = await sequelize.query(`
      SELECT id, "firstName", "lastName" FROM students WHERE displayid IS NULL ORDER BY created_at
    `);
    ...
    const [leavesWithoutId] = await sequelize.query(`
      SELECT id, "leaveType" FROM leave_requests WHERE displayid IS NULL ORDER BY created_at
    `);
